...
Helper.playerItemsCount = (items, steamid) => {
  let count = 0;
  items.forEach((item) => {
    if (item.from_steamID === steamid) count++;
  });
  return count;
};
...
Helper.Generate.roundHashes = () => {
  let percentage = (Math.random() * 100).toFixed(14),
    ...
    hash = crypto
      .createHash('md5')
      .update(percentage.toString() + ':' + secret)
      .digest()
      .toString('hex');
  return {
    percentage,
...
Helper.Generate.offerPin = () => {
  return crypto
    .createHash('md5')
    .update(Date.now().toString() + crypto.randomBytes(5).toString('hex'))
    .digest()
    .toString('hex')
    .slice(0, 8);
};
...
Helper.Item.buildImageURL = (
  classid,
  size = '120fx100',
  appid = config.global.siteAppID
) => {
  return (
    'https://steamcommunity-a.akamaihd.net/economy/image/class/' +
    appid +
    '/' +
    classid +
    '/' +
